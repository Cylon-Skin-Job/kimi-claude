 ...
 * @role Creates the ctx object passed to runtime workspace modules
 *
 * The WorkspaceContext is the ONLY interface between the engine and
 * a workspace's ui/module.js. It provides communication, data access,
 * state management, theming, and DOM helpers.
 */

import { useWorkspaceStore } from '../state/workspaceStore';
import { fetchWorkspaceFile } from '../lib/workspaces';
import type { WorkspaceConfig, WorkspaceTheme } from '../lib/workspaces';
...
export interface WorkspaceContext {
  ...
  /** Request a file from this workspace. Returns content string. */
...
function getContextState(workspaceId: string): ContextState {
  let state = contextStates.get(workspaceId);
  if (!state) {
    state = {
      listeners: new Map(),
      wsHandler: null,
    ...
    contextStates.set(workspaceId, state);
  }
  return state;
}
...
/**
 * Create a WorkspaceContext for a workspace module.
 * Call destroyContext() when the module is unmounted.
 */
export function createContext(config: WorkspaceConfig): WorkspaceContext {
  ...
  const ctx: WorkspaceContext = {
    ...
    emit(type: string, data?: Record<string, unknown>) {
      ...
      socket.send(JSON.stringify({ type, workspace: config.id, ...data }));
    },
    ...
    async request(path: string): Promise<string> {
      ...
      return fetchWorkspaceFile(socket, config.id, path);
    },
    ...
    injectStyles(css: string, id?: string) {
      ...
      // Scope CSS to this workspace's container
      const scoped = css.replace(
        /(^|\})\s*([^@{}][^{]*)\{/g,
        (match, before, selector) => {
          // Don't scope @-rules or already-scoped selectors
          if (selector.trim().startsWith('@') || selector.includes(`[data-workspace="${config.id}"]`)) {
            return match;
          }
          return `${before} [data-workspace="${config.id}"] ${selector.trim()} {`;
        }
      );
...
/**
 * Clean up all listeners, styles, and state for a workspace context.
 * Call this when unmounting a runtime module.
 */
export function destroyContext(workspaceId: string) {
  const ctxState = contextStates.get(workspaceId);
  if (!ctxState) return;
  ...
  contextStates.delete(workspaceId);
}
