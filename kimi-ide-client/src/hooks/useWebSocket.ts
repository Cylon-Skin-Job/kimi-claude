...
export function useWebSocket() {
  ...
  const handleMessage = useCallback((msg: WebSocketMessage) => {
    ...
    switch (msg.type) {
      ...
      case 'content':
        if (msg.text) {
          // Segment store for MessageList compatibility
          ...
          // Route through accumulator (handles boundary detection)
          accumulator.handleContent(msg.text);
        }
        break;
      ...
      case 'thinking':
        if (msg.text) {
          ...
          // Route through accumulator
          accumulator.handleThinking(msg.text);
        }
        break;
      ...
      case 'turn_end': {
        ...
        // Complete all active blocks via accumulator
        accumulator.handleTurnEnd();

        // End turn on queue (marks remaining blocks complete)
        queue.endTurn();
        break;
      }
