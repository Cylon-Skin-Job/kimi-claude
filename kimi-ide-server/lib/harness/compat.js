...
/**
 * Get or create the singleton KimiHarness instance.
 * @returns {KimiHarness}
 */
function getKimiHarness() {
  if (!kimiHarness) {
    kimiHarness = new KimiHarness();
    harnessInitPromise = kimiHarness.initialize({});
    harnessInitPromise.catch(err => {
      console.error('[Compat] Failed to initialize KimiHarness:', err);
    });
  }
  return kimiHarness;
}
...
function getModeStatus(threadId) {
  return {
    ...
    harnessInitialized: kimiHarness !== null,
    activeSessions: kimiHarness ? Array.from(kimiHarness.sessions.keys()) : []
  };
}
...
function emergencyRollback() {
  ...
  if (kimiHarness) {
    kimiHarness.dispose().catch(console.error);
    kimiHarness = null;
