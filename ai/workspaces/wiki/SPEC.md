...
A workspace where wiki pages live as browsable markdown folders. One agent maintains all topics. Work arrives as tickets from the issues workspace. Each ticket becomes a **run** — a folder containing a complete, auditable trace of every step the agent took. The agent traverses the topic graph on demand, follows edges recursively, spawns child tickets for stale edges, and converges when nothing is stale.
...
This workspace follows the universal workspace agent model defined in `ai/workspaces/capture/specs/WORKSPACE-AGENT-SPEC.md`.
...
```
ai/workspaces/wiki/
├── PROMPT.md                ← agent identity (see ai/workspaces/capture/specs/WORKSPACE-AGENT-SPEC.md)
...
**Who reads it:** Everyone. Other agents, other workspaces, skills, humans.
...
One agent for the entire wiki workspace. Not one per topic.
...
```
Reads:   entire project (code, git history, other workspace threads, other wikis)
Writes:  only within ai/workspaces/wiki/ and ai/STATE.md
```
...
2. ai/STATE.md (cross-workspace breadcrumbs)
...
1. Create the folder:
   ```bash
   mkdir -p ai/workspaces/wiki/{topic-name}
   ```

2. Create the two files:
   ```bash
   touch ai/workspaces/wiki/{topic-name}/PAGE.md
   echo "# Change Log" > ai/workspaces/wiki/{topic-name}/LOG.md
   ```

3. (Optional) Add `.slug` if auto-titlecase is wrong:
   ```bash
   echo "CustomSlug" > ai/workspaces/wiki/{topic-name}/.slug
   ```
...
The wiki agent routinely reads from other workspaces and project locations:

| Source | What it reads | Why |
|--------|--------------|-----|
| `ai/workspaces/code/threads/` | Recent coding conversations | Capture decisions made during development |
...
| `ai/STATE.md` | Cross-workspace activity | Know what other workspaces did recently |
| `docs/` | Specs and architecture docs | Cross-reference with wiki content |

This is read-only. The wiki agent writes only within its workspace and to ai/STATE.md.

---

## Relationship to Progressive Disclosure

The wiki workspace is **Layer 3** in the progressive disclosure model:

```
Layer 1: CLAUDE.md        → "a wiki exists at ai/workspaces/wiki/"
...
Tickets give cross-workspace visibility, priority ordering, and natural human intervention (leave a ticket open = block the edit). The wiki agent is a consumer of the issues workspace, not its own task manager. Edge propagation spawns child tickets — the same system handles routing and lifecycle.
...
The wiki workspace has files that don't belong in the GitLab wiki (LOG.md, SPEC.md, index.json, runs/, agent files). The sync script assembles only PAGE.md files. The `.wiki-repo` is git plumbing; the workspace folders are the user interface.
...
3. Add PROMPT.md, TOOLS.md, WORKFLOW.md, workspace.json (copy from template)
