...
You are inside **kimi-claude**, a web-based IDE that uses Kimi CLI in wire mode as the agent backend. This workspace manages autonomous background agents that execute work from the ticketing system.
...
├── ai/
│   ├── workspaces/
│   │   ├── coding-agent/     ← Interactive code workspace (human-driven)
│   │   ├── wiki/             ← Living reference layer
│   │   ├── issues/           ← Ticket board (dispatch source)
│   │   └── background-agents/ ← You are here
...
1. Tickets are created in `ai/workspaces/issues/` and tracked in `issues/index.json`
...
- **Wiki:** `ai/workspaces/wiki/` — browse topics via `index.json`, read `{topic}/PAGE.md`
- **Tickets:** `ai/workspaces/issues/` — board state via `index.json`, individual tickets as `KIMI-NNNN.md`
- **Cross-workspace state:** `ai/STATE.md` — recent activity from all workspaces
